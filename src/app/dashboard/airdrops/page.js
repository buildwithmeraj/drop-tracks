...
function summarizeAccounts(accounts) {
  return accounts
    .map((account) => account.username || account.email || account.wallet || account.label)
    .filter(Boolean)
    .slice(0, 3);
}

export default async function AirdropsPage({ searchParams }) {
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-black tracking-tight md:text-4xl">
            Track every campaign in one list
          </h1>
          <p className="max-w-2xl text-base-content/70">
            Save links, account identifiers, expected dates, and notes for each
            airdrop you are farming.
          </p>
        </div>
                      ...
                      <td className="min-w-56">
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-xs text-base-content/60">
                            ...
                            {item.multiple ? (
                              <span className="badge badge-outline badge-sm">Multiple</span>
                            ) : null}
                          </div>
                          <a
                            href={item.link}
                      ...
                      <td className="min-w-52">
                        <div className="flex flex-wrap gap-2">
                          {accountSummary.length ? (
                            ...
                            <span className="text-sm text-base-content/60">No identifiers</span>
                          )}
                        </div>
                      </td>
                ...
                <tr>
                  <td colSpan="5" className="py-12 text-center">
                    <div className="space-y-3">
                      <p className="text-lg font-semibold">No airdrops tracked yet</p>
